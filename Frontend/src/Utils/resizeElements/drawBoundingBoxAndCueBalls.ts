...
export function drawBoundingBoxAndCueBalls(ctx: CanvasRenderingContext2D, element: elementTypes) {
    if (!ctx) return;
    console.log("inside drawBoundingBoxAndCueBalls");
    const gap = 10;
    ...
    drawCueBalls(ctx, topLeftX, topLeftY, width, height);
}

function drawBoundingBox(ctx: CanvasRenderingContext2D, topLeftX: number, topLeftY: number, width: number, height: number) {
    console.log("inside drawBoundingBox");
    ctx.save();
    ctx.beginPath();
    ctx.lineWidth = 2;
...
function drawCueBalls(ctx: CanvasRenderingContext2D, topLeftX: number, topLeftY: number, width: number, height: number) {
    console.log("inside drawCueBalls");
    const radius = 5;
    const gap = 10;
    const topLeft = {
        x: topLeftX,
...
function drawCueBall(ctx: CanvasRenderingContext2D, x: number, y: number, radius: number) {
    console.log("inside drawCueBall");
    ctx.save();
    ctx.beginPath();
